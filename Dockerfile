...
# Install system deps (nodejs/npm for snarkjs PLONK prove/verify CLI)
...
COPY aimarket-hub/operator.html /app/operator.html
...
COPY aimarket-hub/studio-ui-i18n.json /app/studio-ui-i18n.json
...
RUN pip install --no-cache-dir -e ".[escrow,postgres,pqc]" \
...
# ── PLONK ZK (real proofs — universal setup, no per-circuit ceremony) ──
# Build host must run contracts/zk/scripts/setup_plonk.sh first (deploy_hub.sh
# does this automatically). wasm/zkey live under contracts/zk/build/ (gitignored).
COPY contracts/zk/package.json contracts/zk/package-lock.json /opt/zk/
COPY contracts/zk/scripts/poseidon2.mjs /opt/zk/scripts/poseidon2.mjs
RUN cd /opt/zk && npm ci --omit=dev --silent \
    && ln -sf /opt/zk/node_modules/.bin/snarkjs /usr/local/bin/snarkjs
COPY contracts/zk/verifier/ /app/contracts/zk/verifier/
# The proving key and witness generator are OPTIONAL, and copied as a directory on
# purpose. Naming the two files individually meant a clean clone could not build the image
# at all: both are generated by scripts/setup_plonk.sh (which needs a `circom` binary no
# script here installs) and gitignored, and Docker fails the whole build on a COPY whose
# source is missing. Without them the hub runs on the simulated prover — `make_zk_prover`
# only reaches for real artifacts when AIMARKET_ZK_BACKEND is set — so a missing key is a
# capability the operator has not switched on, not a broken image. The powers-of-tau cache
# and the r1cs are excluded in .dockerignore; they are build scratch, not runtime inputs.
COPY contracts/zk/build/ /app/contracts/zk/build/
...
# Real PLONK defaults — deploy/hub-zk.env sets AIMARKET_ZK_SIMULATED=0 on prod.
ENV AIMARKET_ZK_BACKEND=plonk
ENV AIMARKET_ZK_WASM=/app/contracts/zk/build/input_validity_js/input_validity.wasm
ENV AIMARKET_ZK_ZKEY=/app/contracts/zk/build/input_validity_plonk.zkey
ENV AIMARKET_ZK_VKEY_JSON=/app/contracts/zk/verifier/verification_key.json
ENV AIMARKET_ZK_VERIFIER_SOL=/app/contracts/zk/verifier/Verifier.sol
ENV AIMARKET_ZK_NULLIFIER_DB=/app/data/zk_nullifiers.db
