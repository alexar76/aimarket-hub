...
| `GET` | `/ai-market/v2/federation/peers` | 已知 peers + 信任分 |
...
生产参考： [`../docs/production-modelmarket-dev.md`](https://github.com/alexar76/aicom/blob/main/docs/production-modelmarket-dev.md)

| 检查项 | 动作 |
|----------------|--------|
| TLS | 在 nginx / Caddy 终止 → Hub 容器 |
| Secrets | `AIMARKET_PAYMENT_RECIPIENT`、数据库 URL 经环境变量 — 不进 git |
| Factory 同步 | Cron 或 webhook → `sync_pipeline_mirror_and_hub.py` |
| 插件 | 在 `aimarket serve` 之前 `pip install` 所需插件 |
...
**生态地图：** [Alien Monitor](https://magic-ai-factory.com/monitor/) · [AICOM](https://magic-ai-factory.com)
