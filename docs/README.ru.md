...
| `GET` | `/ai-market/v2/federation/peers` | Известные peers + оценки доверия |
...
Production-референс: [`../docs/production-modelmarket-dev.md`](https://github.com/alexar76/aicom/blob/main/docs/production-modelmarket-dev.md)

| Пункт чеклиста | Действие |
|----------------|--------|
| TLS | Терминация на nginx / Caddy → контейнер хаба |
| Secrets | `AIMARKET_PAYMENT_RECIPIENT`, DB URL через env — не в git |
| Синхронизация Factory | Cron или webhook → `sync_pipeline_mirror_and_hub.py` |
| Плагины | `pip install` нужных плагинов до `aimarket serve` |
...
**Карта экосистемы:** [Alien Monitor](https://magic-ai-factory.com/monitor/) · [AICOM](https://magic-ai-factory.com)
