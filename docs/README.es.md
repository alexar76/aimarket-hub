...
| `GET` | `/ai-market/v2/federation/peers` | Peers conocidos + puntuaciones de confianza |
...
Referencia de producción: [`../docs/production-modelmarket-dev.md`](https://github.com/alexar76/aicom/blob/main/docs/production-modelmarket-dev.md)

| Ítem de checklist | Acción |
|----------------|--------|
| TLS | Terminar en nginx / Caddy → contenedor del hub |
| Secrets | `AIMARKET_PAYMENT_RECIPIENT`, DB URL vía env — no en git |
| Sync Factory | Cron o webhook → `sync_pipeline_mirror_and_hub.py` |
| Plugins | `pip install` de los plugins deseados antes de `aimarket serve` |
...
**Mapa del ecosistema:** [Alien Monitor](https://magic-ai-factory.com/monitor/) · [AICOM](https://magic-ai-factory.com)
