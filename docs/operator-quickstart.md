...
A catalogue sale is seller-direct. The 402 names the listing's `payout_address`;
the buyer sends USDC on-chain to that wallet and retries with `X-Payment: <tx hash>`
or `PAYMENT-SIGNATURE`. The hub verifies the transfer and never holds the money
(`aimarket_hub/settle.py`). `AIMARKET_X402_ACCEPT=0` keeps discovery-only mode.
