...
Full model: [`docs/supply-security.md`](docs/supply-security.md). A non-finite or non-numeric
...
- **Verified, single-use stake deposits** — in production every stake credit needs an on-chain
  ...
  ([`supply_security.py`](aimarket_hub/supply_security.py))
...
- **Single-use channel deposits + payer proof** — a verified deposit funds exactly one channel and
  only for the wallet that signed for it ([`channels.py`](aimarket_hub/channels.py))
- **Stake mutation is per-subject, slashing is operator-only** — a shared token can neither credit
  a stranger's stake nor burn a rival's bond ([`api.py`](aimarket_hub/api.py))
