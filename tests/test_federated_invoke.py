...
def _build(tmp_path: Path, monkeypatch, *, crypto=True, safety_gate=None):
...
def test_legacy_transport_still_settles(tmp_path, monkeypatch):
    ...
    app, db, config = _build(tmp_path, monkeypatch)
    _add_peer(db, LEGACY_PEER)  # no oracle category → no well-known lookup
    ...
    assert r.status_code == 200
    assert out.gets == []  # non-oracle peer: no well-known probe
