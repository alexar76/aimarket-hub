...
def _set_admin_token(monkeypatch):
    ...
    # These hubs run offline (no AIFACTORY_PUBLIC_URL backend); opt into the
    # explicit stub-invoke path so local seeded capabilities execute in CI.
...
class TestCrossHubReputation:
    ...
    def test_submit_reputation_for_peer(self, two_hubs):
        ...
        event = {
            ...
            # Must be CURRENT: since 2026-09-11 the hub rejects a signed event whose
            # timestamp is more than 24h from its own clock. The canonical binds the
            # timestamp, so this is the one replay a peer cannot re-date its way around.
...
class TestWidgetServing:
    ...
    def test_zk_plugin_demo_routes(self, two_hubs):
        """Plugin demo page buttons for aimarket-zk must accept JSON bodies and return stats."""
        client_a, _ = two_hubs
        plugins = client_a.get("/ai-market/v2/plugins").json().get("plugins", [])
        if not any(p.get("name") == "aimarket-zk" for p in plugins):
            pytest.skip("aimarket-zk not loaded in test hub")
        stats = client_a.get("/ai-market/v2/p/aimarket-zk/zk/stats")
        assert stats.status_code == 200, stats.text
        assert stats.json().get("simulated") is True
        prove = client_a.post(
            "/ai-market/v2/p/aimarket-zk/zk/prove-input",
            json={
