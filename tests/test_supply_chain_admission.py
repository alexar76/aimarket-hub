...
def _config(private_key: Ed25519PrivateKey, mode: str = "advisory") -> AdmissionConfig:
    ...
    return AdmissionConfig(
        mode=mode,
        # Documentation TEST-NET — public literal IP that passes the SSRF blocklist.
        auditor_url="https://203.0.113.10/invoke",
...
def test_supply_register_runs_admission_before_catalog_write(
    ...
    monkeypatch.setenv(
        "AIMARKET_SUPPLY_CHAIN_AUDITOR_URL", "https://203.0.113.10/invoke"
    )
...
def test_two_reporters_contradict_a_declaration_and_reach_the_stake_ladder(tmp_path, manifest):
    ...
    assert (final["distinct_reporters"], final["contradicted"]) == (2, True)
