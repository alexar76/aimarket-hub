...
class TestDepositSingleUse:
    ...
    def prod_ledger(self, tmp_db, monkeypatch):
        ...
        monkeypatch.setattr(ch, "_verify_tx_onchain", lambda **kw: {"ok": True})
    ...
    def test_same_tx_hash_funds_only_one_channel(self, prod_ledger):
        first = prod_ledger.open(5.0, wallet="0xAlice", tx_hash="0xdeposit1")
        assert "channel" in first, first
        replay = prod_ledger.open(5.0, wallet="0xAlice", tx_hash="0xdeposit1")
    ...
    def test_distinct_tx_hashes_each_fund_a_channel(self, prod_ledger):
        assert "channel" in prod_ledger.open(5.0, wallet="0xAlice", tx_hash="0xdepA")
        assert "channel" in prod_ledger.open(5.0, wallet="0xAlice", tx_hash="0xdepB")
    ...
    def test_claim_survives_restart(self, tmp_db, monkeypatch):
        ...
        assert "channel" in l1.open(5.0, wallet="0xAlice", tx_hash="0xpersist")
        l1.stop_sweep()
        l2 = ch.ChannelLedger(db_path=tmp_db)
        replay = l2.open(5.0, wallet="0xAlice", tx_hash="0xpersist")
