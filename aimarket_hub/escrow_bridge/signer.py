...
class EnvKeySigner(Signer):
    ...
    def _guard_committed_key(self) -> None:
        ...
        run it is NOT treated as a pass — an environment where the check cannot run gets a
        warning, because the alternative (refusing to sign because git is missing) would
        break a legitimate containerised deploy.
        """
        key = self._key.strip()
        if not key:
            return
        ...
        try:
            found = subprocess.run(
                ["git", "grep", "--fixed-strings", "--quiet", needle],
                cwd=None, capture_output=True, timeout=20, check=False,
            )
        except Exception as exc:
            logger.warning(
