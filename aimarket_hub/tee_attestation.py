...
def _verify_cose_signature(doc: dict[str, Any], cert_chain: bytes) -> bool:
    ...
    try:
        ...
        trusted_store = [root_cert]

        # Build intermediate pool from chain (skip leaf and root)
        intermediates = certs[1:] if len(certs) > 1 else []
...
class TEEAttestation:
    ...
    def verify(self, expected_code_hash: str, enclave_public_key_b64: str) -> bool:
        ...
        # Software fallback: Ed25519 signature verification
        if not enclave_public_key_b64:
            return False
        try:
            from aimarket_hub.signing import Signer
            signer = Signer()
            return signer.verify(enclave_public_key_b64, "", self.canonical())
        except Exception:
            return False
