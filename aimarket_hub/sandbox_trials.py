...
def model_budget_refusal(capability_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "detail": (
            f"{capability_id} composes its answer with a paid model, so every call spends "
            "real budget. The free tier covers capabilities whose marginal cost is noise; this "
            "is not one of them."
        ),
        "how_to_continue": [
...
def trial_policy() -> dict[str, Any]:
    ...
    return {
        ...
        "excluded_reason": "each call spends paid model budget, so it is not free to serve",
