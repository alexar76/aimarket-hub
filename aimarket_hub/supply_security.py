...
class SupplySecurity:
    ...
    def check_invoke_trust(self, cap: Capability) -> None:
        ...
        if publisher_id and _is_unscored(cap.trust_score):
    ...
    def record_permission_violation(
        ...
        *,
        ...
        Deliberately reuses the distinct-reporter rule rather than counting
        reports: one observer filing the same observation repeatedly is one
        voice, exactly as one buyer's repeated failures are in
        ``record_verified_failure``. A signed lie about your own permissions is
        strong evidence — unlike a Metis verdict it does not depend on a
        buyer-supplied intent — so it needs no paid gate, but it still needs
        more than a single accuser.
        """
        publisher_id = (publisher_id or "").strip()
        if not publisher_id:
            return {"slashed": False, "reason": "unknown_publisher"}
        reporter = (consumer_id or "").strip() or "consumer:anonymous"
