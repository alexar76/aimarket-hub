...
from aimarket_hub import contracts_declaration
...
from aimarket_hub.signing import Signer, pqc_required, receipt_signature_version
...
def _catalogue_route(db: HubDatabase, config: HubConfig, capability: Any):
    """Route state shared by every buyer-facing catalogue surface."""
    source_hub = str(getattr(capability, "source_hub", "") or "local")
    peer = None if source_hub == "local" else db.get_peer(source_hub)
    return route_freshness(
        capability,
        peer,
        max_age_s=config.catalog_max_stale_s,
    )


def _catalogue_snapshot(db: HubDatabase, config: HubConfig, *, limit: int = 1000):
    """Fulfillable, public offers whose route is currently sellable."""
    return [
        capability
        for capability in db.list_capabilities(limit=limit)
        if capability_is_fulfillable(capability)
        and capability_is_publicly_offerable(capability)
        and _catalogue_route(db, config, capability).sellable
    ]
...
# The salt is per process unless AIMARKET_BUYER_SALT pins it: a peer needs a buyer id that is
# stable for as long as its rate window, not forever, and an id that cannot be linked across hubs
# or back to an address.
_BUYER_SALT = (os.environ.get("AIMARKET_BUYER_SALT") or "").encode() or secrets.token_bytes(32)


def _opaque_buyer(rate_key: str | None) -> str:
    """An opaque, stable id for one buyer as seen by THIS hub, sent to peers on routed invokes.

    It is an HMAC of the key this hub rate-limits the buyer by (their address, or their sandbox
    visitor id) — never the address, channel or account itself, which are this hub's business.
    A peer can give each buyer their own quota with it; it cannot tell who they are.
    """
    import hmac as _hmac

    digest = _hmac.new(_BUYER_SALT, (rate_key or "anonymous").encode(), hashlib.sha256).hexdigest()
    return "b-" + digest[:20]
...
def create_app(
    ...
    if db is None:
        # Production must never silently fall back to a local SQLite file when an env file
        # is missing or malformed. That creates a fresh, apparently healthy empty market
        # beside the real PostgreSQL ledger — the worst failure mode for balances and trials.
        # Injected databases remain available to isolated tests; the standalone production
        # entry point is fail-closed.
        database_url = (config.database_url or "").strip()
        if _is_production_mode() and not database_url.startswith(("postgresql://", "postgres://")):
            raise RuntimeError(
                "production Hub requires DATABASE_URL=postgresql://...; "
                "SQLite fallback is disabled when AIFACTORY_PROD=1"
            )
        db = HubDatabase(config.db_path, database_url=database_url)
    # The free tier is part of this hub's contract with strangers, so its ledger is built
    # here rather than on the first call that needs it. Lazily, a hub that cannot keep trial
    # counters starts clean, reports healthy, and 500s at the visitor.
    if sandbox_enabled():
        ensure_trial_ledger()
    ...
    hub_settle = settle.PaymentStore(db._conn)
    hub_invoices = settle.InvoiceStore(db._conn)
    ...
    async def _lifespan(fastapi_app: FastAPI):
        ...
        # KI-11: an authorized escrow debit is captured on every paid invoke and, until
        # now, collected only when a person ran the CLI. Off unless an interval is set,
        # and bounded by the bridge's own policy and spend caps rather than by this loop.
        with contextlib.suppress(Exception):
            from aimarket_hub.escrow_bridge import collector as escrow_collector

            if escrow_collector.interval_s() > 0:
                collect_task = asyncio.create_task(escrow_collector.run_forever())
            else:
                logger.debug(
                    "escrow collection: %s", escrow_collector.blocked_reason()
                )
        ...
        try:
            ...
            for task in (crawl_task, collect_task):
                if task is None:
                    continue
                task.cancel()
    ...
    app.add_middleware(
        ...
        allow_headers=[
            ...
            "X-AIMarket-Routing-Fee", "X-AIMarket-Buyer", "X-AIMarket-Crawler", "X-Provider-Signature",
            "A2A-Version", "A2A-Extensions",
        ],
    )
    ...
    async def _x402_middleware(request: Request, call_next):
        ...
        try:
            ...
            # `_bind_listing_payee` always sets this attribute, even to "", so its
            # PRESENCE says the 402 is about a listing and its emptiness says that
            # listing has nobody to pay. Collapsing the two into a falsy string sent
            # the buyer to the operator's own wallet for a sale `settle.terms_for`
            # then refused — the buyer paid the platform and got an error back.
            about_listing = hasattr(request.state, "x402_pay_to")
            pay_to = str(getattr(request.state, "x402_pay_to", "") or "")
            payee_arg = pay_to if about_listing else None
            if (
                about_listing
                and not pay_to
                and (price or 0) > 0
                and str(payload.get("error") or "") == "payment_required"
                and x402.recipient()
            ):
                # Only when this hub HAS an operator wallet, because that is the only
                # case where the buyer was being misdirected. A hub with no wallet at
                # all never named one, so its 402 is plain "payment_required" and
                # saying anything else would change an answer that was already right.
                # No offer can be made, so say why instead of repeating the generic
                # "pay the seller payout_address" — there is no such address, and a
                # buyer who follows that advice to the platform wallet is paying for
                # a call this hub will refuse.
                payload["detail"] = (
                    "this listing is priced but names no payout address, so it cannot "
                    "be sold here; ask the publisher to declare one"
                )
                payload["error"] = "listing_not_sellable"
                return _rebuild(json.dumps(payload).encode("utf-8"))
            v2 = x402.payment_required_v2(price or 0, resource_url, description, pay_to=payee_arg)
            if v2 is None:
                return _rebuild(body)

            # Bind this 402 to one call: the nonce the buyer signs into
            # transferWithAuthorization. Without it a transfer to the seller
            # could have been made for something else.
            cap_id = str(getattr(request.state, "x402_capability_id", "") or "")
            terms_units = 0
            try:
                terms_units = int((v2.get("accepts") or [{}])[0].get("amount") or 0)
            except (TypeError, ValueError):
                terms_units = 0
            if pay_to and terms_units:
                nonce = "0x" + secrets.token_hex(32)
                invoice = hub_invoices.mint(
                    nonce=nonce, capability_id=cap_id, pay_to=pay_to,
                    amount_units=terms_units, ttl_s=settle.invoice_ttl_s(),
                )
                for entry in v2.get("accepts") or []:
                    extra = dict(entry.get("extra") or {})
                    extra["nonce"] = nonce
                    entry["extra"] = extra
                v2["nonce"] = nonce
                payload["nonce"] = nonce
                payload["binding"] = "eip3009"
                payload["pay_to"] = pay_to
                payload["expires_at"] = invoice["expires_at"]

            payload.update(x402.v1_body_fields(
                price or 0, resource_url, description, pay_to=payee_arg,
            ))
            if payload.get("accepts") and pay_to and payload.get("nonce"):
                for entry in payload["accepts"]:
                    extra = dict(entry.get("extra") or {})
                    extra["nonce"] = payload["nonce"]
                    entry["extra"] = extra
            extra_headers = {
            ...
            if payload.get("nonce"):
                extra_headers["X-Payment-Nonce"] = str(payload["nonce"])
            return _rebuild(
    ...
    def _pay_publisher_share(cap: Any, price_usd: float, receipt_id: str) -> None:
        """Credits-rail residual only. Catalogue sales pay the seller on-chain.

        A wallet publisher is paid by ``payout_address``, not by a silent credit.
        This helper remains for a buyer who prepaid in credits: if the publisher
        also holds a credit account, they can be book-kept here. It never
        substitutes for the market rail and never claims a wallet seller
        "cannot be paid".
        """
    ...
    def _bind_listing_payee(request: Request, cap: Any) -> str:
        """Publish the seller on this request so a 402 names their wallet."""
        pay_to = settle.seller_pay_to(cap, operator_pay_to=x402.recipient())
        request.state.x402_pay_to = pay_to
        request.state.x402_capability_id = getattr(cap, "capability_id", "") or ""
        request.state.x402_price = float(getattr(cap, "price_per_call_usd", 0) or 0)
        return pay_to

    def _bind_hub_fee_payee(request: Request, fee_usd: float) -> None:
        """This 402 is for the HUB's routing fee, so it must name the hub's wallet.

        The federated broker path binds the listing's payee early, because most of its
        402s are about the sale. The routing-fee refusal is not: `needed` there is the
        hub's own 1% for brokering somebody else's capability, and it was inheriting the
        seller's address — so the offer asked the buyer to pay the hub's fee TO THE
        SELLER. The hub got nothing and the seller got an extra percent.
        """
        request.state.x402_pay_to = settle.fee_recipient()
        request.state.x402_price = float(fee_usd or 0)

    def _unbind_payee(request: Request) -> None:
        """No single payee can express this offer, so make no on-chain offer.

        A total of "list price plus routing fee" is owed to two different addresses and
        x402's `exact` scheme has one `payTo`. Naming the seller hands them the hub's
        fee; naming the hub makes it custody the seller's price. An empty payee produces
        no `accepts` at all, which leaves the buyer the rails that CAN express it — the
        channel and credit ways already in the body.
        """
        request.state.x402_pay_to = ""

    def _settle_market_payment(
        *,
        raw: str,
        payload: dict | None,
        cap: Any,
        ...
        """Verify the buyer paid this listing's seller on-chain.

        Accepts a bare tx hash (HESTIA) or an x402 PAYMENT-SIGNATURE that
        names the same payee and carries the settle transaction. A signature
        without a chain receipt is not a payment.
        """
        terms = settle.terms_for(cap, operator_pay_to=x402.recipient())
        if terms is None:
            return {"error": "this listing has no seller payout_address"}
        ref = settle.extract_payment_ref(raw, payload)
        tx_hash = ref["tx_hash"]
        nonce = (header_nonce or ref["nonce"] or "").strip().lower()
        inner = (payload or {}).get("payload") if isinstance((payload or {}).get("payload"), dict) else {}
        if (payload or {}).get("authorization") or inner.get("authorization"):
            checked = x402.verify_payment(payload or {}, price_usd=price_usd, pay_to=terms.pay_to)
            if not checked.get("ok"):
                return {"error": checked.get("error") or "payment could not be verified"}
            nonce = nonce or str(checked.get("nonce") or "")
        if not tx_hash:
            return {"error": (
                "pay USDC on-chain to the seller payout_address, then retry with "
                "X-Payment: <tx hash> (and X-Payment-Nonce from the 402)"
            )}
        if hub_settle.seen_tx(tx_hash):
            return {"error": "that payment has already been spent on another call"}
        if nonce and hub_settle.seen_nonce(nonce):
            return {"error": "this authorization has already been used"}

        bind = nonce if (settle.require_binding() and nonce) else ""
        invoice = None
        if bind:
            if not settle.is_nonce(bind):
                return {"error": (
                    "send X-Payment-Nonce with the nonce from this 402 and pay with "
                    "transferWithAuthorization signed over it"
                )}
            invoice = hub_invoices.get(bind)
            if invoice is None:
                return {"error": "unknown payment nonce"}
            if float(invoice.get("consumed_at") or 0) > 0:
                return {"error": "that payment nonce has already been spent"}
            if time.time() > float(invoice["expires_at"]):
                return {"error": "that payment nonce has expired; take a fresh 402"}
            if str(invoice.get("pay_to") or "").lower() != terms.pay_to.lower():
                return {"error": "payment is addressed to somebody else"}
            if int(invoice["amount_units"]) > terms.amount_units:
                terms = settle.PaymentTerms(
                    **{**terms.__dict__, "amount_units": int(invoice["amount_units"])}
                )
        elif settle.require_binding():
            return {"error": (
                "send X-Payment-Nonce with the nonce from this 402 and pay with "
                "transferWithAuthorization signed over it"
            )}
        ...
        if bind and not hub_invoices.consume(bind, settled["tx_hash"]):
            return {"error": "that payment nonce has already been spent"}
        if not hub_settle.claim(
            tx_hash=settled["tx_hash"],
            pay_to=terms.pay_to,
            payer=settled.get("authorizer") or "",
            amount_usd=price_usd,
            amount_atomic=settled["paid_units"],
            asset=terms.token_contract,
            network=terms.chain,
            capability_id=capability_id,
            nonce=bind or settled["tx_hash"],
        ):
            if bind:
                hub_invoices.release(bind)
            return {"error": "that payment has already been spent on another call"}
        logger.info(
            "settle: $%.6f USDC to seller %s for %s tx=%s",
            price_usd, terms.pay_to, capability_id, settled["tx_hash"],
        )
        return {
            "ok": True,
            "tx_hash": settled["tx_hash"],
    ...
    def _release_market_payment(accepted: object) -> None:
        """Un-spend a verified seller-direct payment when the call is refused unserved.

        The money itself cannot come back: this rail pays the seller's wallet directly and
        the Hub is not in the transaction. What the Hub can do, and did not, is stop
        CONSUMING the buyer's claim. `_settle_market_payment` burns both replay keys the
        moment the receipt verifies — `hub_invoices.consume` and `hub_settle.claim` — and
        every refusal after that point (a short input, a safety block, a provider 502, a
        failed capture) left the buyer having paid for a call that never ran, holding a
        nonce and a tx hash that could never be presented again.

        Releasing them restores the only compensation seller-direct can offer: the payment
        stays redeemable, so the buyer retries the same money instead of paying twice. The
        one place that already did this — `hub_settle.claim` failing on an
        already-spent tx — shows the shape was understood; it was just never wired to the
        exits that actually happen.

        Never raises: it runs from `finally`, where an exception would mask the real error.
        """
        if not isinstance(accepted, dict) or accepted.get("error") or not accepted.get("ok"):
            return
        try:
            nonce = str(accepted.get("nonce") or "")
            if nonce:
                hub_invoices.release(nonce)
            tx_hash = str(accepted.get("tx_hash") or "")
            if tx_hash:
                hub_settle.release(tx_hash)
        except Exception as exc:  # noqa: BLE001
            logger.error(
                "invoke: releasing the seller-direct payment %s raised: %s",
                str(accepted.get("tx_hash") or "")[:18], exc,
            )
        else:
            logger.info(
                "invoke: released unserved seller-direct payment %s (nonce %s) — the "
                "buyer's money is with the seller and the claim is redeemable again",
                str(accepted.get("tx_hash") or "")[:18], str(accepted.get("nonce") or "")[:18],
            )
    ...
    def _payment_hint(credits_on: bool, crypto_on: bool) -> str:
        ...
        """
        # With signup closed, POST /accounts answers 403: naming it sent every buyer to a
        # door that refuses them. Keys then exist only as the operator issues them.
        key_source = (
            f"from POST {config.hub_url}/ai-market/v2/accounts"
            if credits.signup_open() else "issued by this hub's operator (signup is closed)"
        )
        if credits_on and crypto_on:
            return (
                "payment required — pay USDC on-chain to the seller payout_address "
                "(X-Payment / PAYMENT-SIGNATURE), or send X-API-Key for a credit account "
                f"({key_source}) or X-Payment-Channel"
            )
        if credits_on:
            return (
                "payment required — pay USDC on-chain to the seller payout_address "
                f"(X-Payment: <tx hash>), or send X-API-Key {key_source}"
            )
        return (
            "payment required — pay USDC on-chain to the seller payout_address, "
            "then retry with X-Payment: <tx hash> or PAYMENT-SIGNATURE"
        )

    def _pays_a_third_party(cap: Any) -> bool:
        """Does settling this listing send money OUT of the hub, to somebody else?

        The credit line is the operator's own resource: money taken in once and issued
        as a limit, most of it granted outright so a new project can try the catalogue
        for free. Spending that limit on a listing whose seller must be paid for real
        would mean the operator pays a stranger, in USDC, out of pocket, for a call the
        buyer got as a gift — and `_pay_publisher_share` cannot even book it, because a
        wallet is not a credit account, so today it is simply dropped and the operator
        keeps 100% of somebody else's sale.

        So the rule is the plain one: a credit line buys the operator's OWN resources.
        Anything with an external payee is bought on the rail that actually pays them.

        `seller_pay_to` is the same resolver settlement uses, so this cannot drift from
        who really gets paid: it answers empty for an operator-owned listing and for a
        named publisher booked inside the ledger, and an address for a listing that
        names one. An address equal to the operator's own recipient is the operator
        selling their own work and is not a third party.
        """
        try:
            payee = settle.seller_pay_to(cap, operator_pay_to=x402.recipient())
        except Exception:  # noqa: BLE001 - never fail an invoke on a policy lookup
            return False
        payee = str(payee or "").strip()
        if not payee:
            return False
        own = str(x402.recipient() or "").strip()
        return payee.lower() != own.lower()
        ...
        ways: list[dict[str, Any]] = [
            {
                "rail": "seller_direct",
                "header": "X-Payment",
                "also": ["PAYMENT-SIGNATURE", "X-PAYMENT", "X-Payment-Nonce"],
                "asset": "USDC",
                "note": "pay the listing payout_address on-chain; the hub never holds the money",
            }
        ]
        if credits_on:
            ways.append({
    ...
    def _declared_workers() -> tuple[int, bool]:
    ...
    _INVOKE_RATE_WORKERS, _INVOKE_RATE_WORKERS_DECLARED = _declared_workers()
    ...
    if _INVOKE_RATE_WORKERS > 1:
        ...
        and not _INVOKE_RATE_WORKERS_DECLARED
    ):
    ...
    async def bazaar_discovery(type: str = "http", limit: int = 20, offset: int = 0):
        ...
        priced = [
            c for c in _catalogue_snapshot(db, config)
    ...
    async def well_known(
        ...
        for p in db.list_peers():
            ...
            # What the peer says it has ON CHAIN, re-exported as it declared it. A reader
            # two hops away cannot fetch this peer (that is what bounds a cyclic
            # federation), so without the re-export a stranger hub is a name and a number
            # — and its escrow, the one thing about it anyone can independently check, is
            # invisible. Re-exported only; never merged with our own declaration, and
            # bounded at crawl time (contracts_declaration.sanitize).
            if p.declared_contracts:
                entry["contracts"] = p.declared_contracts
            peers.append(entry)
        ...
        public_caps = _catalogue_snapshot(db, config)
        ...
        # Where each provider can be REACHED, when that differs from where this hub calls
        # it. A provider is usually registered with the address the hub itself dials — a
        # compose hostname on a single-host deploy — and that address is meaningless to
        # everyone else: `hub.attestedmemory.net` published Memory Market, Truth Layer and
        # Provenance Ledger as `http://memory-market:8810` and friends, so every monitor
        # drew three nodes with no usable link. There was a way to set their NAMES
        # (AIMARKET_ECOSYSTEM_LABELS) and no way to set their addresses.
        #
        #   AIMARKET_ECOSYSTEM_URLS=memory-market=https://memory.example,truth-layer=https://truth.example
        #
        # `=` separates, not `:`, because the value is a URL and contains colons.
        ecosystem_urls: dict[str, str] = {}
        for raw_url in os.environ.get("AIMARKET_ECOSYSTEM_URLS", "").split(","):
            publisher, separator, address = raw_url.partition("=")
            address = address.strip()
            if separator and publisher.strip() and address.startswith(("http://", "https://")):
                ecosystem_urls[publisher.strip()] = address.rstrip("/")[:300]
        ...
        for c in public_local:
            ...
            declared = ecosystem_urls.get(publisher_id, "")
            if declared:
                # The operator has said where this provider actually lives; that wins over
                # anything derivable from how the hub happens to dial it.
                item["url"] = declared
                continue
            ...
            if invoke_url and not item["url"]:
                ...
                address = invoke_url.split("/capabilities/", 1)[0].rstrip("/")
                # A self-published provider registers a bare `…/invoke` with no
                # `/capabilities/` in it, so the split above returned the ENDPOINT as the
                # node address — `http://172.17.0.1:9195/invoke` on the UNI hub. Trim it.
                if address.endswith("/invoke"):
                    address = address[: -len("/invoke")].rstrip("/")
                item["url"] = address
        ecosystem_nodes = []
        ...
        # Configuration readiness is necessary but not sufficient. The live channel
        # stack also needs an RPC/token verifier and payer-signature recovery. Keeping
        # this runtime verdict in discovery prevents a missing optional dependency from
        # advertising a wallet that every channel/open will refuse.
        runtime_payment = payment_readiness()
        payment_ready = bool(
            config.payment_ready and runtime_payment.get("can_credit_channels", False)
        )
        wk = {
            "name": config.hub_name,
            ...
            # A2A 1.0 is a real, bounded marketplace-discovery facade, not just a card.
            # Both URLs are inside this signed document so a consumer of our native
            # protocol can bind the A2A surface to the same Hub identity.
            "a2a_endpoint": f"{config.hub_url}/a2a",
            "a2a_agent_card_url": f"{config.hub_url}/.well-known/agent-card.json",
            ...
            "payment_rails": {
                "seller_direct": {
                    "enabled": x402.enabled() and x402.accept_enabled(),
                    "header": "X-Payment",
                    "also": ["PAYMENT-SIGNATURE", "X-PAYMENT", "X-Payment-Nonce"],
                    "asset": "USDC",
                    "note": (
                        "buyer pays the listing payout_address on-chain; "
                        "this hub verifies and never holds the money"
                    ),
                },
                "credits": {
            ...
            # Federation membership authenticates a Hub, not an arbitrary caller
            # claiming to be one of its agents. Advertise the standard wallet-bound
            # proof endpoint so lottery clients can discover whether their home Hub
            # can attest that second leg of identity.
            "lottery_admission": {
                "version": 1,
                "work_seat_entitlement_url": (
                    f"{config.hub_url}/ai-market/v2/lottery/work-seat-entitlement"
                ),
                "authentication": "Bearer credential for the requested agent_id",
                "participant_id": "sha256(normalized_hub_url + newline + agent_id)",
                "max_ttl_s": 600,
                # Advertise the door an AGENT can actually walk through. _ADMIN_TOKEN is
                # the operator's and can never be presented by an agent, and the shared
                # publish token is refused on this route, so neither makes it "enabled".
                "enabled": bool(_AGENT_TOKENS),
            },
            "peers": peers,
            "ecosystem": {
                "version": 1,
        ...
        # What THIS hub has on chain: the escrow it settles through and the wallet it is
        # paid to, with the chain id and an explorer link, so "payments configured" stops
        # being a claim a reader has to buy something to check. Derived from the payment
        # config and deployment registry already in use — a hub that deploys an escrow
        # starts declaring it with no switch to flip, and one with none stays silent.
        own_contracts = contracts_declaration.own_declaration(
            config, payment_ready=payment_ready
        )
        if own_contracts:
            wk["contracts"] = own_contracts
    ...
    async def v2_manifest():
        ...
        caps = _catalogue_snapshot(db, config)
        ...
        for c in caps:
            ...
            tools.append({
                "name": c.tool_name(),
                "display_name": catalogue_display_name(c),
                ...
                "route_status": _catalogue_route(db, config, c).status,
    ...
    async def search(
        ...
        # ``limit``/``offset`` page the response. They must not hide the catalogue:
        # empty-intent browse ranks the full index, then slices. ``limit=0`` means
        # "from offset to the end". A 100/500 ceiling used to drop live SKUs.
        offset = max(0, int(offset))
        page = int(limit)
        detailed = db.search_capabilities_detailed(intent, limit=None)
        ...
        peer_categories: dict[str, frozenset[str]] = {}

        def _peer_categories(source_hub: str) -> frozenset[str]:
            if not source_hub or source_hub == "local":
                return frozenset()
            if source_hub not in peer_categories:
                try:
                    peer = db.get_peer(source_hub)
                except Exception:  # noqa: BLE001 - a lookup must never fail a search
                    peer = None
                declared = getattr(peer, "categories", None) or []
                peer_categories[source_hub] = frozenset(
                    str(c).strip().lower() for c in declared if isinstance(c, str)
                )
            return peer_categories[source_hub]
        ...
        for cap in results:
            ...
            if not capability_is_publicly_offerable(cap):
                continue
            route = _catalogue_route(db, config, cap)
            if route.status == "unavailable":
                continue
            if route.status == "stale" and not include_stale:
                continue
            if category_key and category_key in _peer_categories(cap.source_hub):
                # The peer's own well-known `categories` — MOMUS declares "security", but
                # none of its ids or descriptions carry a token below, so the ecosystem's
                # security auditor was invisible under category=security.
                pass
            elif category_key:
                blob = (
            ...
            filtered.append({
                ...
                "name": cap.name,
                "display_name": catalogue_display_name(cap),
                ...
                "payout_address": settle.seller_pay_to(
                    cap, operator_pay_to=x402.recipient(),
                ) or None,
                "seller_id": cap.publisher_id or cap.source_hub_name or cap.source_hub,
                "seller_kind": "publisher" if cap.publisher_id else "hub",
                "stake_usd": cap.stake_usd or None,
                "demo": cap.is_demo,
                "offerable": capability_is_publicly_offerable(cap) and route.sellable,
                "route_status": route.status,
                "route_age_s": route.age_s,
                "input_required": input_required,
                "input_hint": input_hint,
            })

        total = len(filtered)
        window = filtered[offset:] if page <= 0 else filtered[offset:offset + page]
    ...
    async def supply_stake(
        ...
        try:
            ...
            # ANY failure, not just ValueError. The debit above has already moved the
            # publisher's money out of their spendable balance into collateral; the stake
            # is only real once `supply_security.stake` has written `supply_stake_add`,
            # `trust_add_edge` and `refresh_publisher_trust`. A sqlite error, a lock
            # timeout, a disconnected Postgres or a cancelled request in the middle of
            # those three left the money taken and no stake recorded, with no path back —
            # `return_collateral` is called from here and nowhere else.
            #
            # Compensating first and re-raising afterwards keeps the original error, which
            # is what the caller needs; the refund is a side effect, not the answer.
            if settled_ref:
                ledger = hub_credits
                if ledger is not None:
                    try:
                        ledger.return_collateral(
                            credit_account, amount, note="stake refused",
                        )
                    except Exception as give_back:  # noqa: BLE001
                        # Never mask the real failure with the failure to undo it — but
                        # never lose it either: this is money nobody else will notice.
                        logger.error(
                            "stake: returning $%.6f collateral to %s after a failed stake "
                            "raised: %s (original failure: %s)",
                            amount, credit_account, give_back, exc,
                        )
    ...
    async def receipt_verify(body: ReceiptVerifyRequest):
        """Verify one receipt against this Hub's live Ed25519 + ML-DSA-65 keys.

        This is deliberately a *self* verifier: the public key is never accepted from
        the request, so a caller cannot make an arbitrary self-signed object appear
        trusted. Browser clients still verify Ed25519 locally; this endpoint evaluates
        ML-DSA-65 until WebCrypto exposes a post-quantum primitive.
        """
        receipt = body.receipt
        # Nginx caps the production request body, but direct ASGI deployments and test
        # clients may not. Keep verification CPU and response size predictably bounded.
        try:
            encoded_size = len(json.dumps(
                receipt, ensure_ascii=False, separators=(",", ":"), default=str
            ).encode())
        except (TypeError, ValueError, RecursionError):
            encoded_size = 131_073
        if encoded_size > 131_072:
            raise HTTPException(status_code=413, detail="receipt exceeds 128 KiB")

        signature = receipt.get("signature")
        version = receipt_signature_version(receipt)
        classical_valid = False
        hybrid_valid = False
        pq_present = isinstance(signature, dict) and bool(signature.get("pq_value"))
        if version and isinstance(signature, dict):
            classical_valid = any(
                Signer.verify(
                ...
                for canonical in signer.receipt_canonical_candidates(receipt, version)
            )
            hybrid_valid = signer.verify_receipt_signature(receipt)

        return {
            "valid": hybrid_valid,
            "classical_valid": classical_valid,
            "pq_present": pq_present,
            # `None` means there was no PQ claim to evaluate. When present, `valid`
            # requires both layers and pins ML-DSA to this Hub's persisted PQ key.
            "pq_valid": hybrid_valid if pq_present else None,
            "algorithm": "ed25519+ml-dsa-65" if pq_present else "ed25519",
            "signature_version": version or None,
            "signer_matches_live_key": classical_valid,
            "policy_requires_pq": pqc_required(),
            "protocol_version": "v2",
        }

    @router.post("/lottery/work-seat-entitlement")
    async def lottery_work_seat_entitlement(
        body: LotteryEntitlementRequest,
        authorization: str = Header(default=""),
    ):
        """Issue a short-lived, Hub-signed lottery entitlement for this agent.

        Every federated Hub uses its own agent credential registry and signing key.
        The lottery relayer trusts only this Hub or an active peer key pinned by its
        federation registry; a self-selected key is never accepted.
        """
        # NOT via the shared-publish-token relaxation. The whole value of this
        # entitlement is "this Hub can tell its agents apart": the relayer turns
        # (hub_url, agent_id) into the on-chain participant nullifier that rations one
        # unpaid work seat per agent. A credential that speaks for ANY agent_id mints
        # unlimited distinct participants from one token — a sybil generator for the
        # lottery — so this route requires a real per-agent credential (or the operator's
        # admin token) on every hub, production or not.
        if not _AGENT_TOKENS and not _ADMIN_TOKEN:
            raise HTTPException(
                status_code=503,
                detail=(
                    "disabled: lottery entitlements need a per-agent credential. Set "
                    "AIMARKET_AGENT_TOKENS='<agent_id>:<secret>,...' (the shared publish "
                    "token cannot prove which agent is asking)."
                ),
            )
        _require_subject_credential(authorization, body.agent_id, (_NS_AGENT,))
        from aimarket_hub.lottery_entitlement import issue_entitlement
        ...
        return {"entitlement": entitlement, "protocol_version": "v2"}
    ...
    async def invoke(
        ...
        # Market rail: a payer who answered our 402 by sending USDC to the
        # seller. The header is a tx hash (HESTIA) or an x402 PAYMENT-SIGNATURE
        # that names the same payee and carries the settle transaction.
        x402_payment = None
        x402_raw = (x_payment or payment or payment_signature or "").strip()
        if x402_raw and x402.accept_enabled():
            if settle.is_tx_hash(x402_raw):
                x402_payment = {"txHash": x402_raw}
            else:
                ...
                if decoded is None:
                    return JSONResponse(status_code=400, content={
                        ...
                        "detail": "the payment header is neither a tx hash, base64 JSON nor JSON",
                        "protocol_version": "v2",
                    })
        ...
        def _refuse_incomplete_input():
            """A 400 naming the fields the caller left out, or None to carry on.

            Asked at both execution points rather than once at the top, and that placement
            is the whole subtlety. Put before the paywall, it answers 400 to an UNPAID call
            on a priced capability — which breaks x402: the client is waiting for a 402 to
            know what to pay, and telling it the input is short instead strands the payment
            flow. `deploy_hub_rebuild.sh` probes exactly that (every selling peer, empty
            input, expects 402) and rolled the apex hub back for it. Payment first, then
            this, then the provider — so nobody is told what they are missing before being
            told what it costs, and nobody crosses the federation to find out either.

            The capability's own `input_schema` is the only source. `anyOf`/`oneOf` are read
            as ALTERNATIVE required groups, because that is how a capability with two ways
            in declares itself — ATLAS's watchbox takes a stored id plus its owner token OR
            a bare bbox, and a flat `required` list cannot say that.
            """
            try:
                described = db.get_capability(body.product_id, body.capability_id)
                if described is None:
                    described = db.find_by_capability_id(body.capability_id)
                absent = missing_required_inputs(
                    body.input, getattr(described, "input_schema", None),
                )
            except Exception:
                # A lookup that fails must not turn a valid call into a 400. The provider
                # still answers, and the classifier still reads its refusal, exactly as
            ...
            stat = InvocationStat(
                ...
                source_hub=body.source_hub or "local",
                price_usd=0.0,
                latency_ms=0,
                success=False,
            ...
            wanted = " or ".join(", ".join(group) for group in absent)
            plural = len(absent) > 1 or len(absent[0]) > 1
            return JSONResponse(status_code=400, content={
                "success": False,
                "error": "incomplete_input",
                "detail": (
                    f"{body.capability_id} requires {wanted} — the request did not include "
                    f"{'them' if plural else 'it'}. This is not a hub or provider failure "
                    f"and is not counted as one."
                ),
                "missing": absent[0] if len(absent) == 1 else absent,
                "accepts": absent,
                "capability_id": body.capability_id,
                "protocol_version": "v2",
            })
        if body.source_hub == "local":
            ...
            list_price = float(cap.price_per_call_usd or 0.0)
            seller_payee = _bind_listing_payee(request, cap)
            if (
                list_price > 0
                and seller_payee
                and x402.accept_enabled()
                and x402.enabled()
                and not sandbox_mode
            ):
                # The catalogue sale is live: this listing has a seller to pay.
                price_rail_active = True
            if body.max_price_usd is not None and list_price > body.max_price_usd:
                return JSONResponse(status_code=409, content={
                    "success": False,
                    "error": "price_limit_exceeded",
                    "detail": "the capability price changed after selection; search again",
                    "maximum_price_usd": body.max_price_usd,
                    "current_price_usd": list_price,
                    "protocol_version": "v2",
                })
            ...
            if x402_payment is not None and cap.price_per_call_usd > 0 and not sandbox_mode:
                x402_accepted = _settle_market_payment(
                    raw=x402_raw,
                    payload=x402_payment,
                    cap=cap,
                    price_usd=cap.price_per_call_usd,
                    capability_id=body.capability_id,
                    header_nonce=(x_payment_nonce or "").strip(),
                )
            ...
            if body.verify is not None:
                ...
                if vs_plan.error:
                    # Also above the try: a bad verification plan is the hub refusing, and
                    # the buyer's payment must not be consumed by that refusal.
                    _release_market_payment(x402_accepted)
                    return JSONResponse(status_code=400, content={
                        "success": False,
            ...
            # ONE call, ONE charge — whichever rail carried it.
            #
            # A 402 advertises every way to pay (`_payment_ways`), and a client that
            # answers it with X-Payment while its standing X-API-Key rides along on the
            # same request — which is simply what an HTTP client does — used to be charged
            # on BOTH: USDC to the seller on chain, and then the full list price held and
            # captured from its prepaid credits. The ledger leg has no refund path, so the
            # second charge was permanent. The same request shape also reached three
            # `return`s between the settlement and the try below, each of which left the
            # on-chain claim consumed for a call that never ran.
            #
            # A verified seller-direct settlement is payment in full. Nothing else is
            # reserved on top of it.
            settled_on_chain = isinstance(x402_accepted, dict) and bool(x402_accepted.get("ok"))

            # A credit line buys the operator's OWN resources. A listing whose seller has
            # to be paid for real is bought on the rail that actually pays them, so the
            # operator never funds a stranger's sale out of a limit they gave away.
            if price > 0 and not settled_on_chain and credit_account and _pays_a_third_party(cap):
                return JSONResponse(status_code=402, content={
                    "success": False,
                    "error": "payment_required",
                    "detail": (
                        "this listing pays its seller directly, so it cannot be bought "
                        "with hub credits — pay on chain with X-Payment"
                    ),
                    "needed": price,
                    **_payment_ways(False, crypto_on),
                    "protocol_version": "v2",
                })

            if price > 0 and not settled_on_chain and credit_account and credits_ledger is not None:
            ...
            elif price > 0 and not settled_on_chain and x_payment_channel:
                ...
                hold_rail = "channel"
                remaining = pay_result.get("remaining_balance", 0)
            elif crypto_on and not settled_on_chain and not sandbox_mode and x_payment_channel:
            ...
            # Payment is settled; before anything is attempted, is the request even complete?
            #
            # This returns ABOVE the try whose `finally` releases everything, and the
            # ordering is deliberate and load-bearing — asked before the paywall it would
            # answer 400 to an unpaid call on a priced capability and break x402, which
            # `deploy_hub_rebuild.sh` probes for. So the release is made here explicitly
            # rather than by moving the gate: a buyer who left out a field has still paid.
            short = _refuse_incomplete_input()
            ...
            trial_visitor = ""
            delivered = False   # the call returned a result to the buyer
            if sandbox_mode:
            ...
            # A capability listed at $0 costs nobody anything, so it takes no trial. It used
            # to: every hub that routes to a peer sends a sandbox visitor header, so a FREE
            # SKU burned the caller's lifetime allowance of three, and from the fourth call on
            # answered 429 `trial_quota_exhausted` — for good. hub.attestedmemory.net routing
            # hunt's free `signal.leaderboard@v1` hit exactly that, and the assay on hunt read
            # the 502 as the attested hub's fault and held it at `review`.
            if sandbox_mode and list_price > 0:
            ...
            try:
                if invoke_url:
                    ...
                    try:
                        ...
                        # misbehavior. A ValueError here is the outbound safety layer
                        # refusing an unsafe/unresolvable destination; it is likewise a
                        # delivery failure, not an uncaught Hub 500 and not slash evidence.
                        # The invoke still fails for the consumer and the hold is released.
                        provider_invoke_failed = True
                        raise HTTPException(
                            status_code=502,
                ...
                if vs_plan is not None and vs_plan.active and vs_plan.paid \
                    ...
                    resolved = True
                    # Credits-rail residual only. A seller-direct settle already
                    # paid the listing's payout_address on chain.
                    if not (x402_accepted and x402_accepted.get("rail") == "seller_direct"):
                ...
                if trial_visitor and not delivered:
                ...
                # The same for a seller-direct payment, which until now had no release at
                # all: `_settle_market_payment` burns both replay keys the moment the
                # receipt verifies, so a provider 502, a safety block, a failed capture or
                # any exception left the buyer having paid for a call that never ran and
                # holding a nonce that could never be presented again. The money is with
                # the seller either way — this returns the CLAIM, so the payment is
                # redeemable on a retry instead of consumed for nothing.
                if not delivered:
                    _release_market_payment(x402_accepted)
        ...
        if fed_cap is not None:
            _bind_listing_payee(request, fed_cap)
        ...
        if body.max_price_usd is not None and fed_authorized_total > body.max_price_usd:
            return JSONResponse(status_code=409, content={
                "success": False,
                "error": "price_limit_exceeded",
                "detail": "the routed price changed after selection; search again",
                "maximum_price_usd": body.max_price_usd,
                "current_price_usd": fed_authorized_total,
                "protocol_version": "v2",
            })
        ...
        # Seller-direct (x402 / X-Payment) on a federated sale this hub bills.
        # The local branch settles this before the provider runs; this branch
        # used to ignore the header and unbind the payee, so search advertised
        # a wallet the invoke then called listing_not_sellable.
        fed_x402_accepted: dict[str, Any] | None = None
        if (
            x402_payment is not None
            and fed_price > 0
            and not sandbox_mode
            and fed_cap is not None
            and (x_payment_channel or credit_account or fed_authorization is not None)
        ):
            # One rail per call. Settling the chain AND holding a channel billed one call
            # twice; skipping the hold instead left an escrow authorization "covering" a
            # routing fee nothing ever reserved, so the fee was never collected. Refused
            # before the tx is claimed, so the buyer's payment stays redeemable.
            return JSONResponse(status_code=400, content={
                "success": False,
                "error": "payment_rails_conflict",
                "detail": "send X-Payment or a payment channel / credit key, not both",
                "protocol_version": "v2",
            })
        if (
            x402_payment is not None
            ...
            fed_x402_accepted = _settle_market_payment(
                raw=x402_raw,
                payload=x402_payment,
                cap=fed_cap,
                price_usd=fed_price,
            ...
            if fed_x402_accepted.get("error"):
                return JSONResponse(status_code=402, content={
                    "success": False,
                    "error": "payment_invalid",
                    "detail": fed_x402_accepted["error"],
                    "needed": fed_price,
                    **_payment_ways(credits_on, crypto_on),
                    "protocol_version": "v2",
                })

        # Resold peers too: this hub pays them out of its own account at the peer, so a
        # visitor that skipped the trial here got unlimited calls on the operator's bill.
        if sandbox_mode and (fed_sells or fed_resells):
            if x_payment_channel:
                return JSONResponse(status_code=400, content={
                    "success": False,
                    "error": "sandbox_conflict",
            ...
            # Free at the source, as catalogued from THIS peer: no trial, same reason as the
            # local branch. An uncatalogued capability (fed_cap None) still takes one — its
            # price is unknown, and waving it through would serve a priced one for nothing.
            if not (fed_cap is not None and fed_list_price <= 0):
                trial = consume_sandbox_trial(x_sandbox_visitor or "", client_ip=client_ip)
                if trial.get("error"):
                    code = 429 if trial["error"] in ("trial_quota_exhausted", "rate_limit_exceeded") else 400
                    return JSONResponse(status_code=code, content={
            ...
            and not credit_account
            and fed_x402_accepted is None
        ):
            # AIMARKET_SELLS_FOR: this hub is the seller of record, fee is 0,
            # one payee. Keep the listing wallet bound so the x402 middleware
            # can mint a nonce and attach accepts. Unbinding here emptied
            # pay_to and the middleware rewrote a real sale into
            # listing_not_sellable (gaia.weather.read / platon.random live).
            # Broker + routing fee still cannot name one payTo — no second
            # payee is invented; that path stays an unbound 402.
            if expected_fee > 0:
                _unbind_payee(request)
            return JSONResponse(status_code=402, content={
                "success": False,
                "error": "payment_required",
                "detail": _payment_hint(credits_on, crypto_on),
                "needed": fed_price if expected_fee <= 0 else fed_authorized_total,
                "price_usd": fed_price,
                "routing_fee_usd": expected_fee,
                **_payment_ways(credits_on, crypto_on),
                "protocol_version": "v2",
            ...
            fed_price > 0 and credit_account and credits_ledger is not None
            and fed_x402_accepted is None
        ):
            # `fed_x402_accepted is None`: a buyer who paid on chain AND sent a key would
            # otherwise be charged twice for one call — the local branch guards the same way.
            ...
            (fed_price > 0 or fed_authorization is not None) and x_payment_channel
            and fed_x402_accepted is None
        ):
        ...
        def _unwind_before_try() -> None:
            """Hand back what is already held on an exit that sits before the `try` below.

            Its `finally` only covers the peer call, so the routing-fee refusals that follow
            left a credit or channel hold for the price in place and burned a verified
            seller-direct claim — the buyer's money in limbo for a call never made.
            """
            if fed_reserved and not fed_settled:
                try:
                    ...
                    logger.error("federated invoke: early release of %s raised: %s", fed_nonce, exc)
                else:
                    if undo.get("error"):
                        logger.error(
                            "federated invoke: early release of %s failed: %s",
                            fed_nonce, undo.get("error"),
                        )
            _release_market_payment(fed_x402_accepted)
        ...
        if expected_fee > 0 and price_rail_active and fed_authorization is not None:
            ...
            if credit_account and credits_ledger is not None:
                ...
                if not fed_escrow_binding:
                    _bind_hub_fee_payee(request, expected_fee)
                _unwind_before_try()
                return JSONResponse(status_code=402, content={
                    "success": False,
        ...
        try:
            ...
            headers: dict[str, str] = {
                ...
                # Every buyer arrives at the peer from this hub's one address, so a peer that
                # rate-limits per address gave all of our buyers one shared bucket: one of them
                # could spend it for everybody. This names the buyer to the peer without saying
                # who they are — see _opaque_buyer.
                "X-AIMarket-Buyer": _opaque_buyer(rate_key),
            }
            ...
            if x402_raw:
                # Seller-direct: the buyer paid the listing's payout_address.
                # Forward the proof so a hearth (HESTIA) can verify the same
                # transfer. Do not substitute a sandbox visitor — that would
                # drop the payment and re-bill the buyer.
                headers["X-Payment"] = x402_raw
                headers["X-PAYMENT"] = x402_raw
                if payment_signature:
                    headers["PAYMENT-SIGNATURE"] = payment_signature
                if x_payment_nonce:
                    headers["X-Payment-Nonce"] = x_payment_nonce
            elif fed_peer_key:
            ...
            # Payment is settled and the hold is taken; before crossing the federation,
            # is the request even complete? A second of round trip to be told "lat/lon
            # required" is a second nobody needs to spend.
            short = _refuse_incomplete_input()
            if short is not None:
                return short
            ...
            schema = None
            with contextlib.suppress(Exception):
                listed_cap = db.get_capability(
                    body.product_id, body.capability_id, source_hub=peer.url,
                )
                if listed_cap is None:
                    listed_cap = db.find_by_capability_id(body.capability_id)
                schema = getattr(listed_cap, "input_schema", None)
            outcome = classify_invoke_outcome(
                delivered=not refused,
                refuse_reason=str(
                    result_data.get("refuse_reason") or result_data.get("error") or ""
                ),
            ...
            if not fed_trial_delivered:
                # Seller-direct has no hold to release — the money went to the seller — but
                # the CLAIM was burned at settlement. The local branch returns it on every
                # undelivered exit; this branch never did, so a peer 502, an incomplete input
                # or a refusal consumed the buyer's tx hash for a call that never ran.
                _release_market_payment(fed_x402_accepted)
            ...
            "pending": [
                {
                    ...
                    "trusted": False,
                    "status": "pending",
                    # TWO counts, because they are two different things and a UI that
                    # shows one of them silently shows the wrong one. `preview_` is what
                    # this hub actually read and signature-checked from the peer's
                    # manifest (capped by AIMARKET_FEDERATION_PREVIEW_MAX_CAPS, so it can
                    # be lower than the truth); `declared_` is the peer's own number out
                    # of its well-known — a claim, never a fact we established. Nothing
                    # is indexed either way.
                    "preview_capabilities": db.count_preview_capabilities(p.url),
                    "declared_capabilities": p.capabilities_count,
    ...
    #: How far a signed event's own timestamp may sit from this hub's clock and still be
    #: recorded. The canonical binds the timestamp, so an old event cannot be re-dated —
    #: but without a window a peer could keep a signed event in a drawer and post it
    #: months later, and the replay check below only sees exact duplicates.
    _REPUTATION_MAX_SKEW_S = 24 * 3600
    _reputation_rate: dict[str, list[float]] = {}
    _REPUTATION_RATE_MAX = 30           # requests (of up to 100 events) per window
    _REPUTATION_RATE_WINDOW_S = 60.0

    def _check_reputation_rate(client_ip: str) -> bool:
        key = (client_ip or "").strip() or "\x00no-address"
        now = time.time()
        window = now - _REPUTATION_RATE_WINDOW_S
        bucket = [t for t in _reputation_rate.get(key, []) if t > window]
        if len(bucket) >= _REPUTATION_RATE_MAX:
            _reputation_rate[key] = bucket
            return False
        bucket.append(now)
        _reputation_rate[key] = bucket
        if len(_reputation_rate) > 4096:
            _trim_rate_buckets(_reputation_rate, window, 4096, keep=key)
        return True

    def _reputation_timestamp_ok(raw: Any) -> bool:
        """ISO-8601 UTC, within ±_REPUTATION_MAX_SKEW_S of now. Unparseable -> False."""
        ...
        return abs((datetime.now(timezone.utc) - parsed).total_seconds()) <= _REPUTATION_MAX_SKEW_S

    @router.post("/reputation/events")
    async def submit_reputation_events(body: ReputationEventsRequest, request: Request):
        """Ingest signed reputation events from federation peers.

        A peer's key proves WHO said it; three more checks decide whether it counts:
        the event's timestamp must be recent (the canonical binds it, so this cannot be
        forged around), the exact signed bytes must not have been recorded before (one
        valid event used to be replayable without limit — 100 per request — and trust.py
        computes success rate and estimated bond from the last 10 events, so a peer
        could set a provider's score by volume), and the caller is rate-limited.
        """
        from aimarket_hub.models import ReputationEvent
        if not _check_reputation_rate(_client_address(request, _TRUSTED_PROXIES)):
            raise HTTPException(
                status_code=429,
                detail=f"reputation submissions limited to {_REPUTATION_RATE_MAX} per "
                       f"{int(_REPUTATION_RATE_WINDOW_S)}s",
            )
        rejected = 0
        replayed = 0
        for ev in body.events:
            ...
            # The signed timestamp must be current. Checked BEFORE the signature so a
            # stale event costs no Ed25519 verification.
            if not _reputation_timestamp_ok(ev.get("timestamp")):
                rejected += 1
                continue
            ...
            stored = db.record_reputation_event(ReputationEvent(
            ...
            if not stored:
                replayed += 1
        return {
            "received": len(body.events) - rejected - replayed,
            "rejected": rejected,
            "replayed": replayed,
        }
    ...
    async def hub_health():
        ...
        return {
            ...
            "version": config.hub_version,
            # Operators and monitors need to prove that production did not silently
            # fall back to a local file after a restart. These are readiness facts, not
            # secrets; keys and connection strings are deliberately not returned here.
            "database_backend": db._backend.backend_type,
            "payment_ready": bool(
                config.payment_ready and runtime_payment.get("can_credit_channels", False)
            ),
            "pqc_ready": bool(signer.pq_public_key_b64),
    ...
    @router.post("/accounts/{account_id}/collateral/return")
    async def return_account_collateral(
        account_id: str, body: dict, authorization: str = Header(default=""),
    ):
        """Give a publisher their posted stake back — OPERATOR ONLY.

        Until this existed, collateral had no way out of the hub at all. `POST
        /supply/stake` moves a publisher's money from `balance_mc` into `collateral_mc`,
        and the ONLY caller of `return_collateral` was the compensation on a stake the
        ledger refused. There is no unstake anywhere in the stack — `supply_security` and
        `database` know `supply_stake_add`, `_get` and `_slash`, and nothing else — so a
        publisher who staked $25 could be slashed out of it or hold it forever, and those
        were the only two endings.

        Deliberately NOT self-service. When a stake may be released is policy — a
        cooldown, whether live listings must be withdrawn first, whether a disputed
        publisher may exit — and inventing that policy inside a bug fix would be worse
        than the hole it closes. This is the same authority that puts money onto the rail
        (`/accounts/{id}/credit`) being able to take it back off collateral, so the
        operator can make a publisher whole today; a self-service unstake is a separate,
        deliberate decision.

        The ledger's own `supply_stakes` row is NOT touched here: this returns the money,
        it does not retire the stake's standing. Pass `note` to say which it was.
        """
        _require_admin(authorization)
        ledger = hub_credits
        if not credits.enabled() or ledger is None:
        ...
        posted = float(account.get("collateral_usd") or 0.0)
        if amount > posted + 1e-9:
            # `return_collateral` floors collateral at zero and would credit the whole
            # amount to spendable balance anyway, inventing money out of a typo.
            raise HTTPException(
                status_code=400,
                detail=f"account holds ${posted:.6f} of collateral, cannot return ${amount:.6f}",
            )
        result = ledger.return_collateral(
            account_id, amount, note=str(body.get("note") or "operator return")[:200],
        )
        if result.get("error"):
            raise HTTPException(status_code=400, detail=result["error"])
        return {"success": True, **result, "protocol_version": "v2"}
    ...
    async def live_stats(limit: int = 50):
        ...
        # Whether this hub can take money at all. A hub that refuses every deposit looks
        # exactly like a hub nobody has paid yet — modelmarket.dev sat in the first state
        # for weeks with 0 refusals in its log, because the deposit door had never once
        # been knocked on. Published next to the volume it governs.
        from aimarket_hub.channels import payment_readiness

        summary["payment_readiness"] = payment_readiness()
        ...
        if x402.accept_enabled():
            # On-chain transfers to sellers this hub verified. Not a receivable.
    ...
    def _register_capital_ipo(r: APIRouter, base: str) -> None:
        ...
        async def capital_ipo(body: IpoRequest, authorization: str = Header(default="")):
            ...
            from aimarket_hub import acex_ai_audit

            ai_pack: dict | None = None
            audit_score = body.audit_score_bps
            auditor_id = ""
            auditor_kind = ""
            pack_sha = ""
            if acex_ai_audit.require_ai_audit():
                if body.audit_score_bps is not None:
                    return JSONResponse(
                        status_code=400,
                        content={
                            "error": "human_audit_override_forbidden",
                            "detail": (
                                "ALP auditor is an agent (ai-auditor:acex-v1). "
                                "Do not submit audit_score_bps; the AI pack is the score."
                            ),
                            "protocol_version": "v2",
                        },
                    )
                caps = acex_ai_audit.caps_for_product(db, body.product_id)
                ai_pack = acex_ai_audit.audit_capabilities(body.product_id, caps)
                if ai_pack.get("verdict") != "approve":
                    return JSONResponse(
                        status_code=400,
                        content={**ai_pack, "error": "ai_audit_rejected", "protocol_version": "v2"},
                    )
                audit_score = int(ai_pack["score_bps"])
                auditor_id = str(ai_pack.get("auditor_id") or acex_ai_audit.AUDITOR_ID)
                auditor_kind = str(ai_pack.get("auditor_kind") or acex_ai_audit.AUDITOR_KIND)
                pack_sha = str(ai_pack.get("pack_sha256") or "")
            res = acex_ipo.float_product(
                ...
                audit_score_bps=audit_score, revenue_share_bps=body.revenue_share_bps,
                auditor_id=auditor_id, auditor_kind=auditor_kind,
                audit_pack_sha256=pack_sha,
            )
            if not res.get("error") and auditor_id:
                try:
                    from aimarket_hub import acex_audit
                    acex_audit.sync_coverage(
                        body.product_id,
                        auditor=auditor_id,
                        cover_usd=float(os.environ.get("ACEX_AI_AUDITOR_COVER_USD", "10000") or 10000),
                        score_bps=int(audit_score or 0),
                        phase="insuring",
                    )
                except Exception:
                    logger.warning("ACEX AI auditor coverage sync failed for %s", body.product_id)
            code = 400 if res.get("error") else 200
            extra = {"ai_audit": ai_pack} if ai_pack else {}
            return JSONResponse(status_code=code, content={**res, **extra, "protocol_version": "v2"})

        @r.get(base + "/listings")
        async def capital_listings(limit: int = 50):
            try:
                return {
                    "listings": acex_ipo.list_listings(limit=min(limit, 500)),
                    "protocol_version": "v2",
                }
            except Exception as exc:
                logger.exception("ACEX listings read failed")
                return JSONResponse(
                    status_code=503,
                    content={
                        "listings": [],
                        "error": "listings_unavailable",
                        "detail": str(exc),
                        "protocol_version": "v2",
                    },
                )
    ...
    from aimarket_hub.mcp_gateway import attach_mcp_discovery_routes, attach_mcp_routes

    def _mcp_caller(request: Any) -> str:
        return _client_address(request, _TRUSTED_PROXIES)

    def _mcp_route_ok(capability: Any) -> bool:
        # The same predicate as the buyer-facing catalogue snapshot: a direct MCP tool is
        # listed only while its capability is fulfillable, public and on a sellable route.
        return (
            capability_is_fulfillable(capability)
            and capability_is_publicly_offerable(capability)
            and _catalogue_route(db, config, capability).sellable
        )
    ...
    attach_mcp_routes(
        mcp_router, db=db, hub_url=config.hub_url, client_address=_mcp_caller,
        search=search, route_ok=_mcp_route_ok,
    )
    ...
    attach_mcp_routes(
        apex_mcp_router, db=db, hub_url=config.hub_url, client_address=_mcp_caller,
        search=search, route_ok=_mcp_route_ok,
    )
    app.include_router(apex_mcp_router)

    # A2A 1.0 marketplace discovery. Pass the existing public search handler rather than
    # duplicating its fulfillment, route-freshness and supply-security policy in an adapter.
    from aimarket_hub.a2a import attach_a2a_routes
    ...
    attach_mcp_discovery_routes(wellknown_router)
    ...
    from aimarket_hub.theme import SITE_CSS, apply_shell
    from aimarket_hub.terminal_ssr import seed_terminal_home, snapshot as landing_snapshot
    ...
    def _load_terminal_home() -> str:
        ...
        try:
            summary, events = landing_snapshot(db, hub_url=config.hub_url, event_limit=12)
            html = seed_terminal_home(html, summary, events)
        except Exception:
            logger.exception("terminal home SSR seed failed; serving the static shell")
        return html
    ...
    # The three webfonts every page's <head> asks for, served by us. They used to come from
    # fonts.googleapis.com, which disclosed each visitor's address to Google before the first
    # byte of content rendered. Filenames carry Google's content hash, so they are immutable.
    fonts_dir = os.path.join(os.path.dirname(__file__), "assets", "fonts")
    if os.path.isdir(fonts_dir):
        app.mount("/assets/fonts", StaticFiles(directory=fonts_dir), name="fonts")
