...
def create_app(
    ...
    async def search(
        ...
        limit = min(limit, 100)
        ranked = db.search_capabilities_ranked(intent, limit=limit * 2)
        # filter_for_discover preserves input order; keep scores aligned by id.
        score_by_id = {c.capability_id: s for c, s in ranked}
        results = supply_security.filter_for_discover([c for c, _ in ranked])
        ...
        for cap in results:
            ...
            filtered.append({
                ...
                "score": score_by_id.get(cap.capability_id, 0.0),
