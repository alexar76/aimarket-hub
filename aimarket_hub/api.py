...
def create_app(
    ...
    def _payment_hint(credits_on: bool, crypto_on: bool) -> str:
        ...
        if credits_on and crypto_on:
            return (
                ...
                f"(POST {config.hub_url}/ai-market/v2/accounts) or X-Payment-Channel"
            )
        if credits_on:
            return (
                "payment required — pay USDC on-chain to the seller payout_address "
                "(X-Payment: <tx hash>), or send X-API-Key from "
                f"POST {config.hub_url}/ai-market/v2/accounts"
            )
        return (
            "payment required — pay USDC on-chain to the seller payout_address, "
    ...
    async def invoke(
        ...
        if sandbox_mode and fed_sells:
            ...
            # price + routing fee is owed to two addresses; one `payTo` cannot say that.
