...
from aimarket_hub.metrics import metrics_payload, record_invoke
    ...
    if well_known:
        try:
            ...
            if resp.status_code == 200:
                advertised = (resp.json() or {}).get("mcp_endpoint")
                if isinstance(advertised, str) and advertised.strip():
                    endpoint = advertised.strip()
...
def create_app(
    ...
    async def invoke(
        ...
        fed_cap = db.find_by_capability_id(body.capability_id)
        ...
        fed_price = 0.0 if (sandbox_mode or not crypto_on or not fed_sells) else fed_list_price
        fed_nonce = f"rcpt_{secrets.token_hex(16)}"
        fed_reserved = False
        fed_settled = False
        ...
        if sandbox_mode and fed_sells:
        ...
        elif crypto_on and fed_price > 0 and not x_payment_channel:
            return JSONResponse(status_code=402, content={
                "success": False,
                "error": "payment_required",
                "detail": "X-Payment-Channel required for paid capability invoke",
        ...
        elif fed_price > 0 and x_payment_channel:
            ...
            fed_hold = hold_channel(
                x_payment_channel, fed_price, receipt_id=fed_nonce,
                secret=x_payment_channel_secret or "",
            )
        ...
        try:
            ...
            if fed_reserved:
                fed_capture = capture_hold(fed_nonce)
            ...
            # The caller is getting a result, so the trial was earned. Set on the way out
            # rather than at the point of consumption, because everything between them can
            # still fail.
            fed_trial_delivered = True
            return result_data
        finally:
            ...
            if fed_reserved and not fed_settled:
                ...
                try:
                    fed_release = release_hold(fed_nonce)
                except Exception as exc:  # noqa: BLE001
                    logger.error(
    ...
    async def supply_permission_violation(
        body: PermissionViolationRequest, request: Request
    ):
        ...
        used to invent a declaration — and one accuser never reaches the
        threshold, because reports are counted by distinct reporter key.
        """
    ...
    @app.post("/studio/run")
    async def studio_run(body: StudioRunRequest):
        ...
        try:
            resp = await post_configured(
                f"{executor}/ai-market/pipelines", json=payload, timeout=120.0
            )
        ...
        # Hand back where the signed bill of materials lives, so the page can link to the
        # verifiable original rather than asking the reader to trust this response.
        if isinstance(out, dict) and out.get("trace_id"):
            out["trace_url"] = f"{executor}/ai-market/pipelines/{out['trace_id']}"
