            ...
            "properties": {
                ...
                "max_price_usd": {
                    ...
                    "description": (
                        ...
                        "price here; the Hub returns price_limit_exceeded before work or "
...
def attach_mcp_routes(
    ...
    *,
    ...
    async def _search(arguments: dict[str, Any]) -> str:
        ...
        for cap in caps:
            ...
            matches.append({
                ...
                # Paste this unchanged into market_invoke. It is deliberately
                # duplicated instead of asking an agent to infer a safety value.
                "max_price_usd": cap.price_per_call_usd,
    ...
    async def _invoke(arguments: dict[str, Any], request: Request) -> tuple[str, bool]:
        ...
        is_error = effective_status >= 400 or (
            isinstance(payload, dict)
            and (payload.get("success") is False or bool(payload.get("error")))
        )
        return json.dumps(payload, indent=2, default=str)[:50_000], is_error
            ...
            try:
                ...
                if name == "market_search":
                    ...
                    text, is_error = await _invoke(
                        arguments if isinstance(arguments, dict) else {}, request
                    )
                else:
                    return _sse(_err(req_id, -32602, f"Unknown tool: {name}"))
                return _sse(
                    _ok(req_id, {"content": [{"type": "text", "text": text}], "isError": is_error})
                )
