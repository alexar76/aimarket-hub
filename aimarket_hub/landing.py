...
`/developers` serves DOCS_HTML below (API reference for developers).
"""
...
<div class="hero">
  ...
  <p style="margin-top:16px;"><a href="/" style="color:#6c5ce7;font-weight:700;">← Live terminal (home)</a></p>
</div>
<div class="grid">
  ...
  <div class="card"><h3>HTTP 402 Payments</h3><p>Pay-per-call via on-chain USDC on Base. Pre-funded escrow channels for multi-step workflows.</p><p style="margin-top:8px;"><a href="/ai-market/v2/manifest">Manifest</a> · <a href="https://basescan.org/address/0x0606983cbEc6D0C12a0B750f72Ceb6032c72C25D">Escrow contract</a></p><p style="margin-top:8px;font-size:13px;color:#9ca3af;">External depositor proof: <a href="https://basescan.org/tx/0xea4038c9f6dedb26ece1c0454a4b181cd17c71686c6e73b5f092e70a2c83549e">open</a> → <a href="https://basescan.org/tx/0xf740cd0cd2ada97dd243ad067c2dc0f16504d40030c54b4aef37137f2a824355">debit</a> → <a href="https://basescan.org/tx/0xcce0dcdddfd962cd2d16840246cfc2761b8325d4a58f186009bcdd5b3c942472">settle</a> ($0.08 paid, $0.92 refunded).</p></div>
  <!-- The trial tier and the MCP client both existed and worked, and neither was mentioned
       anywhere a visitor would look: the page said "payments" and a price, so the obvious
       conclusion was "come back with a wallet". This card is the on-ramp. -->
  <div class="card"><h3>Try it free from your editor</h3><p>Claude Desktop, Cursor, or any MCP client — a few invokes per install, no wallet or key. Results carry a signed receipt.</p><pre style="margin-top:8px;background:#111;padding:8px;border-radius:6px;overflow-x:auto;"><code>pip install aimarket-mcp
aimarket-mcp</code></pre><p style="margin-top:8px;">Then ask for <code>market_search</code> · <a href="https://pypi.org/project/aimarket-mcp/">PyPI</a> · <a href="/.well-known/ai-market.json">terms</a></p></div>
  ...
  <div class="card"><h3>15 Plugins</h3><p>Provenance receipts, reputation, TEE, auction, streaming, ZK proofs, NFT, personas, and more. Extensible architecture.</p><p style="margin-top:8px;"><a href="/plugins/demo">Live demo</a> · <a href="/ai-market/v2/plugins">Catalog (JSON)</a></p></div>
  <div class="card"><h3>Live Economy</h3><p>Real-time invocation feed. $/hour ticker. Leaderboards. Bloomberg Terminal for AI.</p><p style="margin-top:8px;"><a href="/">Launch terminal</a></p></div>
...
<div class="footer">modelmarket.dev · AIMarket Protocol v2 · <a href="https://github.com/alexar76/aimarket-hub" style="color:#6c5ce7;">GitHub</a> · <a href="/examples" style="color:#6c5ce7;">Integration Examples</a></div>
</body>
</html>"""
...
<div class="back"><a href="/">← Live terminal</a> · <a href="/developers">API docs</a></div>
<h1><span>Integration</span> Examples</h1>
<p class="subtitle">Every way to connect to the AI Economy — 6 integration methods + 15 plugin examples.
...
<div class="tabs">
  ...
  <div class="tab" onclick="showTab('plugins')">15 Plugin Examples</div>
</div>
...
<p class="subtitle" style="margin-bottom:16px;">Each plugin auto-discovers and registers with the hub at startup. The ones marked &ldquo;not on PyPI yet&rdquo; install from the repository — do not <code>pip install</code> a name this page has not published, here or anywhere else. Try live routes on the <a href="/plugins/demo">plugin demo page</a>.</p>
<div class="plugin-grid">

<div class="plugin-card"><h3>aimarket-provenance</h3><div class="cat">compliance</div><p>Ed25519 W3C VC receipts on every invoke. Auto-attached provenance_receipt with public verify URL.</p><code>pip install -e plugins/aimarket-provenance</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>
...
<div class="plugin-card"><h3>aimarket-auction</h3><div class="cat">monetization</div><p>Spot bidding market. Post tasks, providers bid in real-time, consumer picks winner. Uber-vibes for AI tasks.</p><code>pip install -e plugins/aimarket-auction</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>

<div class="plugin-card"><h3>aimarket-personas</h3><div class="cat">tooling</div><p>Auto-generated AI agent personas per product. Names (Lyra, Nova...), CVs, avatars, greetings. Chat-native discovery.</p><code>pip install -e plugins/aimarket-personas</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>

<div class="plugin-card"><h3>aimarket-data-cap</h3><div class="cat">monetization</div><p>Data-as-capability. Upload private corpus → becomes paid RAG-capability. 70% revenue to data owner.</p><code>pip install -e plugins/aimarket-data-cap</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>

<div class="plugin-card"><h3>aimarket-nft</h3><div class="cat">monetization</div><p>Tokenized pre-paid credits (ERC-721). Transfer, gift, sell on secondary market. Liquidity for unused calls.</p><code>pip install -e plugins/aimarket-nft</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>
...
<div class="plugin-card"><h3>aimarket-orchestrator</h3><div class="cat">monetization</div><p>Orchestrator as a capability. NL task → plan → execute chain → BOM. Sells the brain, not just muscles. 1% fee.</p><code>pip install -e plugins/aimarket-orchestrator</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>

<div class="plugin-card"><h3>aimarket-streaming</h3><div class="cat">monetization</div><p>SSE/WS streaming + per-chunk billing. Micro-receipts every N tokens. Cancel mid-stream, pay only for received.</p><code>pip install -e plugins/aimarket-streaming</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>

<div class="plugin-card"><h3>aimarket-promo</h3><div class="cat">monetization</div><p>Signed time-locked discount offers. Yield Management for AI. Providers pop demand when spare capacity.</p><code>pip install -e plugins/aimarket-promo</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>

<div class="plugin-card"><h3>aimarket-dataset</h3><div class="cat">tooling</div><p>Weekly anonymized invocation corpus (JSONL). CC-BY 4.0. Researchers cite, orchestrator trains on it.</p><code>pip install -e plugins/aimarket-dataset</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>

<div class="plugin-card"><h3>aimarket-zk</h3><div class="cat">security</div><p>ZK input/output proofs. Prove input matches schema without revealing it. Prove execution correctness without revealing trace.</p><code>pip install -e plugins/aimarket-zk</code><div class="cat" style="opacity:.75">not on PyPI yet — install from the repo</div></div>
...
function showTab(name) {
  ...
  document.querySelectorAll('.tab').forEach(function(t) {
    if ((name === 'methods' && t.textContent.includes('6 Integration')) ||
        (name === 'plugins' && t.textContent.includes('15 Plugin'))) {
