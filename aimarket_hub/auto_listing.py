...
def _maybe_float_on_acex(
    ...
    try:
        ...
        if acex_ai_audit.require_ai_audit():
            pack = acex_ai_audit.audit_capabilities(product_id, capabilities)
            result["ai_audit"] = pack
            if pack.get("verdict") != "approve":
                logger.info("ACEX IPO skipped for %s: AI audit %s", product_id, pack.get("reasons"))
                result["ipo"] = {**pack, "error": "ai_audit_rejected"}
                return
            audit_score_bps = int(pack["score_bps"])
            auditor_id = str(pack.get("auditor_id") or acex_ai_audit.AUDITOR_ID)
            auditor_kind = str(pack.get("auditor_kind") or acex_ai_audit.AUDITOR_KIND)
            pack_sha = str(pack.get("pack_sha256") or "")
        else:
            # Legacy: success-rate proxy. Production default is ACEX_REQUIRE_AI_AUDIT=1.
