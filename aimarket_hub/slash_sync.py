...
def attestation_tier(env: dict[str, Any], verifier: Signer) -> str | None:
    ...
    try:
        ...
        return None
    if att.pom is not None and att.pom.verify(verifier):
        return "strong"
    return "weak"
...
class SlashRegistry:
    ...
    def __init__(self, hub_url: str, db: Any = None):
        ...
        # key -> 'strong' | 'weak' (locally-authored entries are first-hand → strong).
    ...
    def _load(self) -> None:
        ...
        for row in rows:
            ...
            self._tiers[(issuer, seq)] = str(row.get("tier") or "strong")
    ...
    def record_local_slash(
        ...
        *,
        ...
        if self._db is not None:
            ...
            seq, env = self._db.slash_attestation_append(self.hub_url, _build, "strong")
        else:
        ...
        # Our own slashes are first-hand knowledge: strong locally. Peers ingesting
        # this envelope classify it by its PoM, not by our self-assessment.
        self._tiers[key] = "strong"
        return env
    ...
    def slash_signal(self, provider_hub: str) -> dict[str, Any]:
        ...
        strong_issuers = {
            ...
            if self._tiers.get(key, "strong") == "strong"
        }
